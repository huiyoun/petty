...
Petty does not create a new agent. It sends your message to the Telegram chat where your agent already lives, then displays incoming agent messages in the macOS pet UI.
...
- `API ID`
- `API Hash`
- `Session Path`, 보통 `~/.petty/telegram-user`
- `Chat ID`, `list-chats`에서 확인한 값
- `Reply From`, 그룹에서는 권장되는 선택 항목
